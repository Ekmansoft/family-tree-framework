...
export const TreeView: React.FC<UnifiedTreeViewProps> = ({
  ...
  if (layoutId === 'ancestor') {
    ...
    }, [individuals, families, selectedId, maxAncestors, horizontalGap, boxHeight, boxWidth, verticalGap]);
  ...
  // Vertical view path
  // Default distances to boxHeight if not provided
  ...
  if (selectedId) {
          ...
          if (fam) {
            ...
            (fam.children || []).forEach((siblingId: string) => {
              ...
              siblingFamilies.forEach(siblingFamId => {
                ...
                if (sibFam) {
                  (sibFam.parents || []).forEach((spouseId: string) => { reachableIndividuals.add(spouseId); });
                  (sibFam.children || []).forEach((childId: string) => { reachableIndividuals.add(childId); });
                }
              });
            });
          }
    ...
    for (let gen = 0; gen < maxGenerationsForward; gen++) {
      ...
      currentGeneration.forEach(personId => {
        ...
        childFamilies.forEach(famId => {
          ...
          const fam = familyById.get(famId);
          if (fam) {
            (fam.parents || []).forEach((parentId: string) => { reachableIndividuals.add(parentId); });
    ...
    const result = filterByMaxTrees({ individuals, families, selectedTreeIndex: undefined, focusItemId: null });
  ...
  }, [individualsLocal, familiesLocal, levelOf, siblingGap, parentGap, familyPadding, maxGenerationsForward, maxGenerationsBackward, boxWidth, boxHeight, effectiveFamilyToParentDistance, effectiveFamilyToChildrenDistance]);
...
export const LegacyTreeView: React.FC<TreeViewProps> = ({
    individuals,
    families = [],
    selectedId,
    onSelectPerson,
